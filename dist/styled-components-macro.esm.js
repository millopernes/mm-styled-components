...
var allowedImports = Object.keys(styled$1).filter(function (helper) {
  return helper !== '__esModule';
});
...
function styledComponentsMacro(_ref) {
  ...
  Object.keys(references).forEach(function (refName) {
    if (!allowedImports.includes(refName)) {
      throw new MacroError('Invalid import: ' + refName + '. You can only import ' + allowedImports.join(', ') + ' from \'styled-components/macro\'.');
    }
